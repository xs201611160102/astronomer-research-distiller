...
assert corpus_audit["ads_raw_unique_bibcodes"] == 4
assert corpus_audit["ads_identity_filtered_bibcodes"] == 1
assert corpus_audit["audit_manifest_records"] == 3
...
audit_manifest = json.loads((project / "metadata/correspondence_audit_manifest.json").read_text())
