...
17. PDF 下载器必须优先尝试 ADS identifier 暴露出的 arXiv PDF，再尝试 ADS gateway 和配置回退链接；arXiv 默认不设置总下载时间限制，下载过程应输出逐条进度，并通过 gateway 短超时、单记录超时和单记录最大尝试数控制出版社拒绝或慢链接造成的长尾等待。
...
18. The PDF downloader must try arXiv PDFs exposed by ADS identifiers before ADS gateway and configured fallback links. ArXiv downloads have no total-time cap by default; gateway, per-record, and max-attempt controls prevent refused or slow publisher links from stalling the corpus.
